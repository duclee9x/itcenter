...
for key in APP_IMAGE POSTGRES_IMAGE CADDY_IMAGE APP_VERSION GIT_COMMIT BUILD_TIME RUNTIME_ENV_FILE DATABASE_URL_FILE \
  ...
  AGENT_GATEWAY_HEALTH_URL AGENT_GATEWAY_HEALTH_RESOLVE GUEST_BACKUP_MOUNT LIMA_WORK_DIR \
  BACKUP_AGE_RECIPIENT BACKUP_MOUNT_FSTYPES POSTGRES_MAJOR_VERSION BACKUP_ESCROW_AGE_IDENTITY_REF \
...
if [[ "$environment" == production ]]; then
  ...
  [[ "$(value BACKUP_ESCROW_AGE_IDENTITY_REF)" != REPLACE_* && "$(value BACKUP_ESCROW_AGENT_CA_REF)" != REPLACE_* ]] || die RECOVERY_ESCROW_REFERENCE_REQUIRED
fi
