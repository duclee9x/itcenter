...
| `TASK-094` | `F-050` | `WF-017/WF-INT01` | P5 | P2 | Risk + Replacement Scoring | TASK-038, TASK-050, TASK-058, TASK-059, TASK-094-R1 | **BLOCKED** | NOT_STARTED | `TASK-094_ASSET_RISK_REPLACEMENT_SCORING.md` |
...
- **TASK-094-R1 — Risk + Replacement Scoring Contract:** `SATISFIED / CODE_COMPLETE` (normative contract persisted; no runtime scoring implemented).
- **TASK-094 — Risk + Replacement Scoring:** `BLOCKED / NOT_STARTED` by `SCOPE_DEPENDENCY`: Maintenance classification, TASK-059 application command/port, and Asset `risk_state` compatibility must be resolved as recorded in the detailed contract.
...
```text
CURRENT = TASK-094 (BLOCKED / NOT_STARTED; explicit SCOPE_DEPENDENCY recorded in TASK-094_ASSET_RISK_REPLACEMENT_SCORING.md.)
NEXT = Stop. TASK-094-R1 is complete. Do not implement TASK-094 or begin TASK-095 until the owning-domain dependencies are resolved.
...
TASK-094 = BLOCKED / NOT_STARTED (SCOPE_DEPENDENCY: TASK-038 corrective/preventive classification, reusable TASK-059 candidate application port, and separate canonical Asset missing/presence signal before `risk_state` becomes a derived score projection)
```
