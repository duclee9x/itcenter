...
`READY / NOT_STARTED` after completion of its edge security contract; runtime
implementation and staging evidence remain pending. Overall readiness remains
`BLOCKED_FOR_RC`.
...
| RR-05 | DATA_MIGRATION_GAP       | High     | RELEASE-006 implementation is complete, but no qualifying production-like prior-schema upgrade, lock/duration measurement, N-1 compatibility, or forward-recovery rehearsal is evidenced. | Yes                                                                       |
...
| RR-07 | SECURITY_GAP             | Critical | RELEASE-007-R1 fixes certificate lifecycle, trusted-proxy, numeric rate/body/timeout and failure-policy decisions; Caddy/API edge implementation and staging evidence remain absent.      | Yes                                                                       |
