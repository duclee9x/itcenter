...
| RELEASE-004      | Immutable Build / Staging / Promotion / Rollback Pipeline |       P0 | `BLOCKED`       | `BLOCKED`            | None                            | `SPEC_GAP / OPERATIONAL_DECISION`; see [R1](items/RELEASE-004-R1_IMMUTABLE_ARTIFACT_DEPLOYMENT_PROMOTION_CONTRACT.md). |
...
`VERIFIED`. RELEASE-004 is `BLOCKED / NOT_STARTED` because the repository has
no normative deployment target, registry, promotion authority, migration
ordering, or rollback contract. [RELEASE-004-R1](items/RELEASE-004-R1_IMMUTABLE_ARTIFACT_DEPLOYMENT_PROMOTION_CONTRACT.md)
records this decision gap; no pipeline implementation is authorized until it
is resolved. Overall readiness remains `BLOCKED_FOR_RC`.
