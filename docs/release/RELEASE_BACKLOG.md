...
| RELEASE-007      | Production TLS Ingress + Rate Limiting                    |       P0 | `NOT_STARTED`   | `READY`              | RELEASE-004                     | R1 is complete. Implement Caddy TLS/redirect, API middleware limits, bounded proxy trust and edge verification.  |
...
qualifying N-1 and production-like verification remain open. RELEASE-007 is
`READY / NOT_STARTED` after completion of
[RELEASE-007-R1](items/RELEASE-007-R1_PRODUCTION_EDGE_TLS_RATE_LIMITING_CONTRACT.md).
Its implementation must preserve direct Agent mTLS and the Podman/Lima
topology; production-like edge verification remains a later gate.
