...
After token validation, resolve exact `(issuer, subject)` through canonical IdentityLink, parse exactly one required `X-Tenant-ID`, resolve an ACTIVE TenantMembership to the tenant-local User, check local user state, and use local RBAC/effective permissions before resource authorization. The header is a client-selected context only: membership and authorization are still mandatory. An unlinked but valid external identity receives 403 `IDENTITY_NOT_PROVISIONED`; missing/inactive tenant membership or insufficient permission receives 403. Missing header is 400 `TENANT_CONTEXT_REQUIRED`; malformed, empty or duplicate header is 400 `INVALID_TENANT_CONTEXT`. A syntactically valid tenant without an active membership receives a safe 403 indistinguishable from an unknown tenant. IdP role/group/tenant claims do not directly authorize platform actions and cannot override the header. Domains receive a canonical tenant-bound principal and never parse the raw token.
...
All API routes require authentication by default and are classified as PUBLIC, PROTECTED_TENANT_SCOPED or explicitly PROTECTED_NON_TENANT. Tenant-scoped business routes require `X-Tenant-ID`; an omitted classification is never public. The initial public health allow-list is `GET /api/v1/health/live` and `GET /api/v1/health/ready`; those requests ignore tenant headers and do not construct tenant principals. `/api/v1/health/capabilities` is protected unless a later explicit contract changes that decision. Missing/invalid production OIDC config prevents serving protected traffic and keeps auth readiness false; no test/mock/bypass adapter is permitted in production. Public health responses contain no sensitive diagnostics.
