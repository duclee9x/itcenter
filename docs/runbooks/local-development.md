...
Run migrations before starting apps. API/gateway readiness checks a platform table with a zero-row query; missing schema or unavailable DB returns a sanitized 503. Worker liveness is available, but readiness is 503 until delivery adapters are configured. SIGINT/SIGTERM closes HTTP listeners and pools; shutdown has a 10-second bound.
