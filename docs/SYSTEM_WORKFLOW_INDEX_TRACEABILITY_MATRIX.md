...
ASSET.RETURNED
ASSET.MISSING (Offboarding Asset recovery; not Asset Risk)
ASSET.RETIRED
ASSET.DISPOSED
```
...
TASK-090 owns rule decisions and durable intents only. Action execution/result
events (`AUTOMATION.STARTED`, action outcomes, retries, rollback and
completion) belong to TASK-091 or an explicitly earlier execution workflow.
